...
4. 两种菜单都只选择近战武器，并通过 `FusedWeaponData` 把武器 ID、战斗属性、附魔类型身份和等级写入戒指 `modData`；数值以固定格式持久化，并兼容旧的逗号小数格式。
5. `RingCombatManager.Update` 从已装备戒指读取熔铸签名；签名变化时刷新战斗属性和临时熔铸武器缓存，冷却结束且范围内存在存活怪物时执行一次 360 度光环攻击，并在本次攻击内按原武器 ID 重建武器、注册附魔以触发原版伤害与击杀回调。
...
- 熔铸数值必须是有限值，伤害区间、攻击范围和武器类型必须有效；解析失败时清空战斗缓存，禁止沿用上一枚戒指的光环数据。
- 配置数值在读取和保存时必须处于 GMCM 公布范围内；GMCM 重置原地恢复同一配置对象，保证已创建的战斗管理器立即使用默认值。
